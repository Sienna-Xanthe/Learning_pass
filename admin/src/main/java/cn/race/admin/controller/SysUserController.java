...
import cn.race.admin.common.dto.PassDto;
...
//	BCryptPasswordEncoder passwordEncoder;
	...
	public Result save(@Validated @RequestBody SysUser sysUser) {
		...
		// 默认密码
		String password = MD5Util.encode(Const.DEFULT_PASSWORD);
	...
	public Result repass(@RequestBody Long userId) {
		...
		sysUser.setPassword(MD5Util.encode(Const.DEFULT_PASSWORD));
		sysUser.setUpdated(LocalDateTime.now());
...
//		boolean matches = passwordEncoder.matches(passDto.getCurrentPass(), sysUser.getPassword());
		boolean matches = MD5Util.encode(passDto.getCurrentPass()).equals(sysUser.getPassword());
		if (!matches) {
			return Result.fail("旧密码不正确");
		}

		sysUser.setPassword(MD5Util.encode(passDto.getPassword()));
		sysUser.setUpdated(LocalDateTime.now());
